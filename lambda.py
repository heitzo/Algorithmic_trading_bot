...
def validation(age, investment_amount):
    if age is not None:
        ...
        if age > 65:
            return build_validation_result(False, "age", "Your age must be lower than 65")
        elif age < 0:
            return build_validation_result(False, "age", "Please provide a valid age")
    
    if investment_amount is not None:
        investment_amount = parse_int(investment_amount)
        if investment_amount < 5000:
            return build_validation_result(False, 'investmentAmount', 'Minimum to invest is $5000')
    
    return build_validation_result(True, None, None)

investment_recommendations = {'none':'100% bonds (AGG), 0% equity(SPY)','low':'100% bonds (AGG), 0% equity(SPY)',
'medium':'100% bonds (AGG), 0% equity(SPY)', 'high':'100% bonds (AGG), 0% equity(SPY)'}
...
def recommend_portfolio(intent_request):
    ...
    if source == 'DialogCodeHook':
        slots = get_slot(intent_request)
        
        validation = validation(age, investment_amount)
        ...
        output_session_attributes = intent['sessionAttributes']
        return delegate(output_session_attributes, get_slots(intent_request))
        
    recommendation = investment_recommendations[risk_level.lower]
    
    return close(intent_request['sessionAttributes'],
    'Fulfilled',
    {'contentType': 'PlainText',
        'content': f'Based on the chosen level of risk, we recommend the folowing distribution: {recommendation}'
    })
